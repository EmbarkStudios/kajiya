...
[[vk::binding(0)]] Texture2D<float4> blur_pyramid_tex;
[[vk::binding(1)]] Texture2D<float4> self_tex;
...
void main(uint2 px: SV_DispatchThreadID, uint2 px_within_group: SV_GroupThreadID, uint2 group_id: SV_GroupID) {
    float4 pyramid_col = blur_pyramid_tex[px];
    ...
    for (int y = -k; y <= k; ++y) {
        for (int x = -k; x <= k; ++x) {
            ...
            self_col += self_tex.SampleLevel(sampler_lnc, uv, 0);
        }
    }
    ...
    //float4 self_col = self_tex[px / 2];
    float4 self_col = self_tex.SampleLevel(sampler_lnc, uv, 0);
#endif
