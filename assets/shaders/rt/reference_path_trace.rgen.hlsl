...
float3 sample_metalness_albedo_boost_lut(float metalness, float3 diffuse_albedo) {
    return float3(
...
void main() {
    ...
    for (uint path_length = 0; path_length < MAX_PATH_LENGTH; ++path_length) {
        ...
        if (primary_hit.is_hit) {
            ...
            const float3 albedo_boost = sample_metalness_albedo_boost_lut(gbuffer.metalness, gbuffer.albedo);
