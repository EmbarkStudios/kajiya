...
void age_ircache_entry(uint entry_idx) {
    ...
    if (is_ircache_entry_life_valid(new_age)) {
        ...
        ircache_grid_meta_buf.InterlockedAnd(sizeof(uint2) * cell_idx + sizeof(uint), ~IRCACHE_ENTRY_META_OCCUPIED);
    }
}
...
void main(uint entry_idx: SV_DispatchThreadID) {
    ...
    if (entry_idx < total_entry_count) {
        if (ircache_entry_life_needs_aging(ircache_life_buf[entry_idx])) {
