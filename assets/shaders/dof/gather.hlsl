...
static const float RAD_SCALE = 1.0; // Smaller = nicer blur, larger = faster

[numthreads(8, 8, 1)]
void main(uint2 px: SV_DispatchThreadID) {
    float2 uv = float2(px + 0.5) / float2(1920, 1080);
	...
	for (float ang = 0.0; radius < max_blur_size; ang += GOLDEN_ANGLE) {
		float2 tc = uv + float2(cos(ang), sin(ang)) / float2(1920, 1080) * radius;
	...
	for (float i = 0; i < sample_count; ++i) {
		...
		float2 tc = uv + float2(cos(ang), sin(ang)) / float2(1920, 1080) * r;
		float3 sampleColor = color_tex.SampleLevel(sampler_lnc, tc, 0);
		float sampleDepth = depth_tex.SampleLevel(sampler_lnc, tc, 0);
		float sampleSize = abs(coc_tex.SampleLevel(sampler_lnc, tc, 0));
