...
float3 lookup_csgi(float3 pos, float3 normal, CsgiLookupParams params) {
    ...
    for (uint gi_slice_iter = 0; gi_slice_iter < GI_SLICE_COUNT; ++gi_slice_iter) {
        ...
        const float3x3 slice_rot = build_orthonormal_basis(params.slice_dirs[gi_slice_idx].xyz);
            ...
            if (params.use_grid_linear_fetch) {
                ...
                if (all(gi_uv == saturate(gi_uv))) {
                    ...
                    if (params.use_pretrace) {
                        radiance = params.alt_cascade0_tex.SampleLevel(sampler_lnc, gi_uv, 0).rgb;
                    } else {
                        radiance = params.cascade0_tex.SampleLevel(sampler_lnc, gi_uv, 0).rgb;
                    }
                }
            } else {
                // Nearest lookup
                ...
                if (gi_vx.x >= 0 && gi_vx.x < GI_VOLUME_DIMS) {
                    if (params.use_pretrace) {
                        radiance = params.alt_cascade0_tex[gi_vx + int3(GI_VOLUME_DIMS * gi_slice_idx, 0, 0)].rgb;
                    } else {
                        radiance = params.cascade0_tex[gi_vx + int3(GI_VOLUME_DIMS * gi_slice_idx, 0, 0)].rgb;
                    }
                }
            }

            const float3 ldir = params.slice_dirs[gi_slice_idx].xyz;
            const float visibility = saturate(0.0 + dot(mul(ldir, normal), float3(0, 0, -1)));
