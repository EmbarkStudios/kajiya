...
void main() {
    const uint3 px = DispatchRaysIndex().xyz;

    const float3 slice_dir = mul(slice_rot, float3(0, 0, -1));
    
    const float3 trace_origin = vx_to_pos(px + float3(0, 0, 0.5));

    RayDesc outgoing_ray;
    outgoing_ray.Direction = vx_to_pos(px + float3(0, 0, -1)) - trace_origin;
    ...
    else*/
    {
        ...
        float3 scatter = 0.0;
        //scatter += out0_tex[int3(px) + int3(0, 0, -1)].xyz;
        ...
        for (uint dir_i = 0; dir_i < DIR_COUNT; ++dir_i)
        {
            ...
            float3 neighbor_pos = vx_to_pos(float3(px) + dirs[dir_i] * 1.5);    // 1.5 to hit corner/back wall
    ...
    if (primary_hit.is_hit) {
        ...
        scatter += out0_tex[int3(px) + dirs[dir_i]].rgb * weights[dir_i];
    }
