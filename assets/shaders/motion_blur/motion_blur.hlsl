...
void main(uint2 px: SV_DispatchThreadID) {
    ...
    const float2 tile_velocity = blur_scale * tile_velocity_tex[clamp(px + tile_offset, int2(0, 0), int2(output_tex_size.xy) - 1) / 16];
    ...
    //if (length(tile_velocity * output_tex_size.xy) > kernel_width * 4)
    {
    ...
    int2 center_px = clamp(int2(center_uv * output_tex_size.xy), int2(0, 0), int2(output_tex_size.xy - 1));

    //float3 center_color = texelFetch(inputImage, px, 0).rgb;
    float3 center_color = input_tex[center_px];
    float center_depth = -depth_to_view_z(depth_tex[center_px]);
    float2 center_velocity = blur_scale * velocity_tex[center_px];
    float2 center_velocity_px = center_velocity * output_tex_size.xy;
    ...
    if (length(tile_velocity) > 0) {
        for (int i = 1; i <= kernel_width; ++i) {
            ...
            int2 px0 = int2(uv0 * output_tex_size.xy);
            int2 px1 = int2(uv1 * output_tex_size.xy);

            float d0 = -depth_to_view_z(depth_tex[px0]);
            float d1 = -depth_to_view_z(depth_tex[px1]);
            float v0 = length(blur_scale * velocity_tex[px0] * output_tex_size.xy);
            float v1 = length(blur_scale * velocity_tex[px1] * output_tex_size.xy);

            float weight0 = sample_weight(center_depth, d0, length((uv0 - uv) * output_tex_size.xy), length(center_velocity_px), v0, soft_z);
            float weight1 = sample_weight(center_depth, d1, length((uv1 - uv) * output_tex_size.xy), length(center_velocity_px), v1, soft_z);
#if 1
            ...
            sum += float4(input_tex[px0].rgb, 1) * weight0;
            sum += float4(input_tex[px1].rgb, 1) * weight1;
        }
